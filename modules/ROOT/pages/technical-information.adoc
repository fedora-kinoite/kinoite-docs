...
This page provides some background technical information on Kinoite, including
...
who are interested in the technical details or those who want to use Kinoite in
...
to compose, deploy and update Kinoite. ostree operates in a similar manner to a
...
makes it possible to install RPMs on Kinoite.
...
On Kinoite, the root filesystem is immutable. This means that `/`, `/usr` and
everything below it is read-only.

`/var` is where all of Kinoite's runtime state is stored. Symlinks are used to
